from trainers.trainer import BaseTrainer, cAETrainer, GMMEstTrainer
...
def get_trainer(optimizer, cfg):
    ...
    elif trainer_type == "cAE":
        trainer = cAETrainer(optimizer, cfg["training"], device=device)
    elif trainer_type == "gmmest":
        trainer = GMMEstTrainer(optimizer, cfg["training"], device=device)
    return trainer
