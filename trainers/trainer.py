...
class cAETrainer:
    ...
    def train(self, model, d_dataloaders, logger=None, logdir=""):
        ...
        for i_epoch in range(1, cfg['n_epoch'] + 1):
            for x, tau in train_loader:
                i_iter += 1

                model.train()
                start_ts = time.time()
                d_train = model.train_step(x.to(self.device), tau.to(self.device), optimizer=self.optimizer, **kwargs)
                time_meter.update(time.time() - start_ts)
                logger.process_iter_train(d_train)
                ...
                model.eval()
                if i_iter % cfg.val_interval == 0:
                    for x, tau in val_loader:
                        d_val = model.validation_step(x.to(self.device), tau.to(self.device))
                        logger.process_iter_val(d_val)
                    d_val = logger.summary_val(i_iter)
                    val_loss = d_val['loss/val_loss_']
                    print(d_val['print_str'])
...
class GMMEstTrainer:
    ...
    def train(self, model, d_dataloaders, logger=None, logdir=""):
        ...
        for i_epoch in range(1, cfg['n_epoch'] + 1):
            for tau, mu, cov in train_loader:
                i_iter += 1
